...
The Pico program is in [`micropicoMills/main.py`](micropicoMills/main.py). The current program includes the Shelly activity webhooks, AS5600 diagnostic sampling, manual calibration capture, Homebridge state reporting, and provisional settled-slot reporting. Record metadata belongs to Now-Playing's `Mills Playlist` rather than the Pico firmware.
...
the normal idle confirmation ends the Mills session. If power remains active,
the candidate is reported as a real slot 20 selection.
